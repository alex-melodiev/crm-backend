...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Region.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Region.forge(params).save(data, { path: true });
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Region._attributes)
      .filter(attribute => attribute !== Region.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Region._attributes[attribute].type));
